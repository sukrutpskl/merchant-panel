...
const email = `seeder${Math.floor(Math.random() * 10000)}@merchant.com`;
...
async function run() {
    ...
    await request('/api/Auth/register', {
        method: 'POST',
        body: JSON.stringify({ firstName: 'Seed', lastName: 'User', email, phone: '555' + Math.floor(Math.random() * 10000000), password, role: 2 })
    });
    ...
    await request('/api/Merchants', {
        method: 'POST', body: JSON.stringify({ name: 'Yemek & Kozmetik Dünyası', description: 'Catalog Seeder', phone: '5551234567', address: 'Seed Addr', minimumOrderAmount: 10, deliveryFee: 5, autoAcceptOrders: true, preparationTimeMinutes: 20 })
    }, token);
    ...
    for (const c of categories) {
        ...
        if (cRes.status === 200 && cRes.data && cRes.data.data) {
            ...
            console.log(`Success: ${c.name} -> ID: ${cRes.data.data.id}`);
        } else {
            console.error(`Failed to create category: ${c.name}`, cRes);
        }
    }
    ...
    for (const catName of Object.keys(products)) {
        ...
        for (const p of products[catName]) {
            console.log(`Creating product: ${p.name} under ${catName}`);
            ...
            console.log(`Response for ${p.name}:`, pRes.status, pRes.status === 200 ? 'OK' : pRes.data);
        }
    }
